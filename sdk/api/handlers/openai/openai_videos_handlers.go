...
func (s *videoAuthBindingStore) set(videoID string, authID string, model string, prompt string, ttl time.Duration) {
	if s == nil {
		return
	}
	...
	if existingPrompt == "" {
		if existing, ok := s.entries[videoID]; ok && now.Before(existing.expiresAt) {
...
func (s *videoAuthBindingStore) get(videoID string) (string, bool) {
	authID, _, ok := s.getWithModel(videoID)
	return authID, ok
}

func (s *videoAuthBindingStore) getWithModel(videoID string) (string, string, bool) {
	if s == nil {
		return "", "", false
	}
	videoID = strings.TrimSpace(videoID)
	...
	return entry.authID, entry.model, true
}

func (s *videoAuthBindingStore) getPrompt(videoID string) (string, bool) {
	if s == nil {
		return "", false
	}
	videoID = strings.TrimSpace(videoID)
...
func (h *OpenAIAPIHandler) bindVideoAuthIDFromPayload(payload []byte, authID string, model string, prompt string) {
	videoID := videoIDFromPayload(payload)
	if videoID == "" {
		return
	}
	videoAuthBindings.set(videoID, authID, model, prompt, h.videoAuthBindingTTL())
}
...
func videoModelFromBinding(videoID string) string {
	if _, model, ok := videoAuthBindings.getWithModel(videoID); ok && model != "" {
		return model
	}
	return defaultXAIVideosModel
}
...
func buildXAIVideosRetrieveResponse(respPayload []byte, rawRequest []byte, videoProxyURL string, videoID string, prompt string) []byte {
...
func (h *OpenAIAPIHandler) VideosRetrieve(c *gin.Context) {
	...
	resp, upstreamHeaders, errMsg := h.ExecuteWithAuthManager(cliCtx, xaiVideosHandlerType, videoModelFromBinding(videoID), payload, "")
	stopKeepAlive()
	if errMsg != nil {
	...
	videoAuthBindings.set(videoID, selectedAuthID, videoModelFromBinding(videoID), "", h.videoAuthBindingTTL())
...
func (h *OpenAIAPIHandler) downloadXAIVideoContent(c *gin.Context, videoID string) {
	...
	stopKeepAlive := h.StartNonStreamingKeepAlive(c, cliCtx)
	videoModel := videoModelFromBinding(videoID)
	resp, _, errMsg := h.ExecuteWithAuthManager(cliCtx, xaiVideosHandlerType, videoModel, payload, "")
	stopKeepAlive()
	if errMsg != nil {
		h.WriteErrorResponse(c, errMsg)
		if errMsg.Error != nil {
	...
	videoAuthBindings.set(videoID, selectedAuthID, videoModel, "", h.videoAuthBindingTTL())
...
func (h *OpenAIAPIHandler) collectXAIVideosNative(c *gin.Context, rawJSON []byte, model string, bindCreatedVideoAuth bool) {
	...
	if bindCreatedVideoAuth {
		...
		model = videoModelFromBinding(videoIDFromPayload(rawJSON))
	}
	stopKeepAlive := h.StartNonStreamingKeepAlive(c, cliCtx)
	...
	if bindCreatedVideoAuth {
		h.bindVideoAuthIDFromPayload(resp, selectedAuthID, model, strings.TrimSpace(gjson.GetBytes(rawJSON, "prompt").String()))
		...
		resp = buildXAIVideosRetrieveResponse(resp, rawJSON, videoProxyURL, videoID, prompt)
	}
	handlers.WriteUpstreamHeaders(c.Writer.Header(), upstreamHeaders)
...
func (h *OpenAIAPIHandler) collectXAIVideosCreate(c *gin.Context, xaiReq []byte, meta xaiVideoCreateMetadata) {
	...
	h.bindVideoAuthIDFromPayload(out, selectedAuthID, meta.Model, meta.Prompt)
	handlers.WriteUpstreamHeaders(c.Writer.Header(), upstreamHeaders)
	_, _ = c.Writer.Write(out)
	cliCancel(nil)
}
